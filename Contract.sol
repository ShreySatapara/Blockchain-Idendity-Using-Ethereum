...
contract Indent{
   ...
   //struct verify{
    address[] user_verification_req;
        //string number;
    uint[] typee_verification_req;
...
//request function for various documents
    ...
    function req_voter(address user_address) public{
        main storage m1 = mainindex[user_address];
        //string memory add = m1.pannumber_main;
        m1.EPICnumber[m1.pannumber_main].req_voter.push(msg.sender);
    }
...
//request verification 
    function requestVerification(uint typeee) public{
        user_verification_req.push(msg.sender);
        typee_verification_req.push(typeee);
        //verifyindex[msg.sender] = verify(msg.sender,number1,typeee);
    }


    function getVerificationRequest() public onlyVerifire returns(address[] memory, uint[] memory){
        return(user_verification_req,typee_verification_req);
...
//verify the data
    ...
    /* 1. Addhar    
        ...
        int index_add=-1;
        int index_type=-2;
        for(uint i=0;i<user_verification_req.length;i++){
            if(user_add==user_verification_req[i]){
                index_add = int(i); //0
            }
            if(doc_type==typee_verification_req[i]){
                index_type = int(i); //
            }
            if(index_add==index_type){
                for (uint j = uint(index_add); j<user_verification_req.length-1; j++){
                    user_verification_req[j] = user_verification_req[j+1];
                }
                delete user_verification_req[user_verification_req.length-1];
                user_verification_req.length--;
                
                for (uint j = uint(index_type); j<typee_verification_req.length-1; j++){
                    typee_verification_req[j] = typee_verification_req[j+1];
                }
                delete typee_verification_req[typee_verification_req.length-1];
                typee_verification_req.length--;
                
                //delete user_verification_req[uint(index_add)];
                //delete typee_verification_req[uint(index_type)];
                break;
            }
        }
        main storage m1 = mainindex[user_add];
